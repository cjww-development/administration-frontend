...
class MessagesVerificationSpec extends PlaySpec {

  val lines = Source.fromFile("conf/messages").getLines().toList

  private def key(line: String): String = line.takeWhile(_ != '=')
  def findErrors(regex: String): List[String] = {
    lines.map(line => if(line.matches(regex)) key(line.trim) else "").distinct.filterNot(_.eq(""))
  }
